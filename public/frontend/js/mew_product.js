        ...
        if (variant.available) {
            ...
            if (variant.price === 0) {
                ...
                if (variant.compare_at_price > variant.price) {
                    comparePrice
                        .html(
                            formatStoreCurrency.format(variant.compare_at_price)
                        )
                        .removeClass("d-none");
                } else {
                    comparePrice.addClass("d-none");
            ...
            if (variant.inventory_policy === "continue") {
                ...
                buyNow.removeClass("d-none").addClass("d-flex");
                inventoryStat
                    .text(variantStrings.allowBuyWhenSoldOut)
                    .addClass("text-warning")
                    .removeClass("text-success text-danger");
            } else {
                inventoryStat
                    .text(variantStrings.soldOut)
...
$(function() {
    $("#product-selectors").length &&
        mewService.getProductJson(productAlias).then(res => {
            ...
            if (
                ...
                !res.options[0].name.includes("Title")
            ) {
                $(".selector-wrapper")
                    .eq(0)
                    .prepend(`<label>${res.options[0].name}</label>`);
            }
            if (
                res.variants.length <= 1 &&
                res.options[0].name.includes("Default")
            ) {
    ...
    myModalEl.addEventListener("show.bs.modal", function(event) {
        if (
            ...
            $(this)
                .find(".embed-responsive")
                .html(
                    `<iframe class="embed-responsive-item" src="https://www.youtube.com/embed/${videoId}?enablejsapi=1" allowfullscreen></iframe>" allow='autoplay; encrypted-media' allowfullscreen></iframe>`
                );
        }
    });
