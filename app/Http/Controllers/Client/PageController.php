...
class PageController extends Controller
{
    ...
    public function listServiceCate($cate)
    {
        $data['services'] = Services::where(['status'=>1, 'category_slug'=>$cate])->select()->paginate(12);
        $data['serviceCate'] = ServiceCategory::where(['status'=>1, 'slug'=>$cate])->first();
        $data['title_page'] = languageName($data['serviceCate']->name);
        $data['news'] = Blog::where(['status'=>1])->orderBy('id', 'desc')->limit(5)->get(['id', 'title', 'slug', 'image']);
        $data['discountPro'] = Product::where('status', 1)->where('discount', '>', 0)->limit(5)->get(['id', 'name', 'price', 'discount', 'images', 'cate_slug', 'type_slug', 'slug']);
        // dd($data['discountPro']);
        return view('service.list', $data);
