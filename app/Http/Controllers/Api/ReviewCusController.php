...
class ReviewCusController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        if($keyword == ""){
            $data = ReviewCus::orderBy('id','DESC')->get(['id','name','content','created_at','phone','license_plate']);
        }else{
            $data = ReviewCus::where('phone', 'LIKE', '%'.$keyword.'%')->orWhere('license_plate', 'LIKE', '%'.$keyword.'%')->orderBy('id','DESC')->get()->toArray();
        }
        return response()->json([
            'data' => $data,
            'message' => 'success'
